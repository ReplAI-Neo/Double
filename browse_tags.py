    ...
    with driver.session() as session:
        ...
        for i, r in enumerate(conversations, 1):
            ...
            for cat_name in sorted(tag_dict.keys()):
                if cat_name in ['formality', 'tone', 'sentiment', 'friendliness', 'purpose', 
                               'complexity', 'resolution', 'engagement', 'domain', 'urgency']:
    ...
    while True:
        ...
        if choice == '1':
            ...
            try:
                ...
                tag_name = input(f"\nEnter tag name (or number 1-{len(tags)}): ").strip()
